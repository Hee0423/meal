...
<html>
	...
	<div class="search-options">
		<form id="searchForm"
			...
			method="post">
			<input type="hidden" name="actiontype" value="search" /> <input
				...
				<tbody>
					<tr>
						<td>商品名称 <input name="name" value="${name}" class="input-txt"
							...
							<web:dropdownlist id="sptype"
								cssclass="dropdown" datasource="${sptype_datasource}"
								textfieldname="mingcheng" valuefieldname="id">
							</web:dropdownlist>
			...
			<c:forEach var="temshangpin" items="${listshangpin}">
				<tr>
					...
					<td><a class="orange-href"
						...
						<a target="_blank" class="chakan"
						href="../e/shangpininfo.jsp?id=${temshangpin.id}">查看</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
