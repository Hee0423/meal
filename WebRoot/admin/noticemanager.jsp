			    ...
			    $("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			             ...
			             $.dialog.confirm("你确定要注销公告信息?", function(){
				             window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/noticemanager.do?actiontype=delete&id='+id);
				          });
			        });
			    });
	  ...
	  <body >
			   ...
			   <div class="search-title">
                ...
                <div class="description">
                    <a href="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do?actiontype=load&seedid=207">新建公告</a>
                </div>
              </div>
					<!-- 搜索控件开始 -->
					  <div class="search-options">
					  <form id="searchForm"  action="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do" method="post" >
					...
					<div class="toolbar-box">
		             ...
		             <table id="module" width="100%" border="0" cellspacing="0" cellpadding="0" class="ui-record-table">
						...
						<c:forEach var="temnotice" items="${listnotice}">
							<tr>
							    <td>
					&nbsp<input id="chk${temnotice.id}"  class="check"   name="chk${temnotice.id}" type="checkbox"  value='${temnotice.id}' >
							    </td>
					<td >${temnotice.title}</td>
																        <td >${temnotice.pubren}</td>
																	 <td >${temnotice.pubtime}</td>
																     <td >${temnotice.clickcount}</td>
								<td >
								     <a class="edit" href="<%=SystemParam.getSiteRoot() %>/admin/noticemanager.do?actiontype=load&id=${temnotice.id}&seedid=207">修改</a>
					                 <a  target="_blank"  href="<%=SystemParam.getSiteRoot() %>/e/noticeinfo.jsp?id=${temnotice.id}&seedid=207">查看</a>
								</td>
							</tr>
							</c:forEach>
						</tbody>
