...
public class HuiyuanAction extends PageActionBase {
	...
	private void exit() {
		...
		if (request.getSession().getAttribute("shanghu") != null) {
			...
			System.out.println("商户退出");
			request.getSession().removeAttribute("shanghu");
	...
	private void login() {
		...
		if (usertype != null && usertype.equals("1")) {
			shanghuLogin(request, response);
		}

	}

	private void shanghuLogin(HttpServletRequest request,
		...
		Shanghu shanghu = (Shanghu) DALBase.load("shanghu", filter);
		...
		if (shanghu != null && shanghu.getPassword().equals(password)) {
			...
			try {
				...
				if (forwardurl!=null)
					...
					response.sendRedirect(SystemParam.getSiteRoot() + "/e/shanghu/accountinfo.jsp");
				}
			} catch (IOException e) {
	...
	public void save() {
		...
		huiyuan.setName("");
		huiyuan.setZhiye("");
		huiyuan.setAihao(kouwei==null?"":kouwei);
		huiyuan.setJifen(10);
		huiyuan.setStatus(1);
		huiyuan.setYue(0);
