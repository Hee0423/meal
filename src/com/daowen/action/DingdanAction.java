...
private void qianshou() {
	 ...
	 * @return
	 */
	private Hashtable<String,List<Dingdanitems>> groupItems(){
      ...
      String spid=request.getParameter("spid");
         ...
         for(Iterator<Dingdanitems> it= temlist.iterator();it.hasNext();)
         {
	       	  ...
	       	  if(ddi.getSpid()==new Integer(spid))
	       	  {
	       	       ...
	       	       it.remove();
	       	       double totalfee=Double.parseDouble(request.getSession().getAttribute("totalfee").toString());
	...
	private void shopcart() {
		...
		List<Dingdanitems> cart = (List<Dingdanitems>) request.getSession().getAttribute("cart");
		if(cart==null)
        {
		...
		if (spid != null) {
			...
			if (!hasin) {
				...
				System.out.println("商品编号="+addshangpin.getSpno());
				temitem.setSpid(addshangpin.getId());
				temitem.setShuliang(1);
				//商户
				temitem.setShname(addshangpin.getPubren());
				temitem.setHyjia(addshangpin.getHyjia());
				temitem.setJiage(addshangpin.getJiage());
	...
	public void save() {
		...
		//分单
		Hashtable<String, List<Dingdanitems>> hashtable=groupItems();
		
		for(String  keyname : hashtable.keySet() )
		
		{
			List<Dingdanitems>  listitems=hashtable.get(keyname);
			if(listitems==null){
				try {
	        ...
	        for(Dingdanitems dd : listitems){
				...
				totalprice+=Double.valueOf(dd.getHyjia())*dd.getShuliang();
